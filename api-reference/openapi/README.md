...
| Synced from | branch `seed/initial-openapi-specs`, commit `6fe2fa2` |
...
- **Interim (now):** copy the changed `openapi/*.yaml` from `fincra-openapi` into this
  folder and open a docs PR. Update the commit hash in the table above.
- **Target:** the `publish-to-docs` workflow in `fincra-openapi` opens that PR
  automatically on each tagged release, so this copy is generated, not hand-maintained —
  the spec repo stays the single source of truth.
